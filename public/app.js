...
/* ── TRANSLATION ENGINE (fast, no API key, full-coverage) ────────────────── */
...
const TRANSLATION_QUOTA_KEY = "mymemory_translation_disabled_until";

function getTranslationQuotaDisabledUntil() {
  const value = Number(localStorage.getItem(TRANSLATION_QUOTA_KEY) || 0);
  return Number.isFinite(value) && value > Date.now() ? value : 0;
}

function setTranslationQuotaDisabledUntil(msFromNow) {
  const disabledUntil = Date.now() + msFromNow;
  localStorage.setItem(TRANSLATION_QUOTA_KEY, String(disabledUntil));
  return disabledUntil;
}

function parseTranslationQuotaWarning(details) {
  if (!details || typeof details !== "string") return 0;
  const match = details.match(/(\d+)\s+hours?.*?(\d+)\s+minutes?.*?(\d+)\s+seconds?/i);
  if (!match) return 0;
...
const SPEECH_LANGS = {
  English:"en-IN", Assamese:"as-IN", Bengali:"bn-IN", Bodo:"brx-IN", Dogri:"doi-IN",
  Gujarati:"gu-IN", Hindi:"hi-IN", Kannada:"kn-IN", Kashmiri:"ks-IN", Konkani:"kok-IN",
  Maithili:"mai-IN", Malayalam:"ml-IN", Manipuri:"mni-IN", Marathi:"mr-IN", Nepali:"ne-IN",
  Odia:"or-IN", Punjabi:"pa-IN", Sanskrit:"sa-IN", Santali:"sat-IN", Sindhi:"sd-IN",
  Tamil:"ta-IN", Telugu:"te-IN", Urdu:"ur-IN", "Indian Sign Language":"en-IN"
};
...
async function myMemoryTranslate(text, langCode) {
  ...
  if (getTranslationQuotaDisabledUntil()) return text;
  try {
    ...
    const responseDetails = json?.responseDetails || "";
    const responseStatus = Number(json?.responseStatus || 0);
    if (responseStatus >= 400 || /used all available free translations|usage limit|quota/i.test(responseDetails)) {
      const disabledFor = parseTranslationQuotaWarning(responseDetails) || (60 * 60 * 1000);
      setTranslationQuotaDisabledUntil(disabledFor);
      return text;
    }
...
async function translateBatch(texts, language) {
  ...
  try { console.debug && console.debug(`[translateBatch] language=${language} texts=${texts.length}`); } catch(e){}
  const langCode = MYMEMORY_LANGS[language];
  if (getTranslationQuotaDisabledUntil()) return {};
  if (!langCode) {
    texts.forEach(tx => translationMisses.add(`${language}:${tx}`));
    try { console.debug && console.debug(`[translateBatch] no langCode for ${language}, marking misses for ${texts.length} texts`); } catch(e){}
    return {};
  }
  ...
  // Fire up to 10 requests in parallel for much faster bulk translation
  const CHUNK = 10;
  const allPromises = segments.map(({ original, query }) => {
    const key = `${language}:${query}`;
    if (_inFlight.has(key)) return _inFlight.get(key).then(r => ({ ...r, original }));
  ...
  for (let i = 0; i < allPromises.length; i += CHUNK) {
    const results = await Promise.all(allPromises.slice(i, i + CHUNK));
  ...
  try { console.debug && console.debug(`[translateBatch] completed language=${language} translated=${Object.keys(map).length}`); } catch(e){}
  return map;
}

// Collect ALL translatable content from the app (UI strings + curriculum + questions)
function getAllAppTexts() {
  const texts = new Set(window.I18N.staticTexts());

  // Add curriculum lesson titles and summaries
  if (state.data?.curriculum) {
    state.data.curriculum.forEach(lesson => {
      if (lesson.title) texts.add(lesson.title);
      if (lesson.summary) texts.add(lesson.summary);
      if (lesson.levels) {
        lesson.levels.forEach(level => {
          if (level.title) texts.add(level.title);
          if (level.description) texts.add(level.description);
          if (level.sections) {
            level.sections.forEach(section => {
              if (section.title) texts.add(section.title);
              if (section.description) texts.add(section.description);
            });
          }
        });
      }
  ...
  // Add path/lesson topic names
  if (state.data?.paths) {
    state.data.paths.forEach(path => {
      if (path.title) texts.add(path.title);
      if (path.audience) texts.add(path.audience);
      if (path.topics) path.topics.forEach(t => texts.add(t));
    });
  }

  return [...texts].filter(t => t && typeof t === "string" && t.length > 1);
}

// Pre-translate ALL question bank content for a given language
// This runs once at language selection; subsequent renders are instant from cache
async function preTranslateQuestions(language) {
  if (language === "English") return;
  try {
    // Fetch question bank from server to get all questions upfront
    const res = await fetch("/api/questions-all");
    if (!res.ok) return;
    const allQuestions = await res.json();
    const texts = new Set();
    allQuestions.forEach(q => {
      if (q.prompt) texts.add(q.prompt);
      if (q.explanation) texts.add(q.explanation);
      if (q.topic) texts.add(q.topic);
      if (Array.isArray(q.options)) q.options.forEach(o => texts.add(o));
      if (Array.isArray(q.correctAnswers)) q.correctAnswers.forEach(a => texts.add(a));
    });
    await translateBatch([...texts], language);
    try { console.debug && console.debug(`[preTranslateQuestions] queued ${texts.size || texts.length} question texts for ${language}`); } catch(e){}
...
// prepareLanguage: translates ALL app content (UI + curriculum + questions)
// Shows a loading overlay so the user sees one wait instead of constant re-renders
let languageWarmupPromise = null;
let languageWarmupTarget = null;

function prepareLanguage(language, opts = { full: false }) {
  if (!language || language === "English") return Promise.resolve();
  if (languageWarmupPromise && languageWarmupTarget === language) return languageWarmupPromise;

  languageWarmupTarget = language;
  languageWarmupPromise = (async () => {
    const staticTexts = window.I18N.staticTexts();
    await translateBatch(staticTexts, language);
    ...
    const curriculumTexts = getAllAppTexts().filter(t => !staticTexts.includes(t));
    // If full warmup requested, await curriculum and question pre-translation
    if (opts.full) {
      await translateBatch(curriculumTexts, language);
      await preTranslateQuestions(language);
    } else {
      void translateBatch(curriculumTexts, language);
      void preTranslateQuestions(language);
    }
  })().catch(() => {}).finally(() => {
    if (languageWarmupTarget === language) {
      languageWarmupPromise = null;
      languageWarmupTarget = null;
    }
  });
  return languageWarmupPromise;
}
...
async function saveProfile(source) {
  ...
  const previousAvatarDraft = state.avatarDraft;
  const previousSettingsAvatarDraft = state.settingsAvatarDraft;
  const previousLanguage = state.data?.profile?.preferredLanguage || "English";
  const payload = profilePayload(source);
  const nextLanguage = payload.preferredLanguage || "English";
  const shouldRelocalizeSession = !!state.session && previousLanguage !== nextLanguage;
  ...
  const translationPromise = previousLanguage !== nextLanguage
    ? prepareLanguage(nextLanguage)
    : Promise.resolve();

  try {
    const data = await api("/api/profile", { method:"POST", body:JSON.stringify(payload) });
    setData(data);
    if (shouldRelocalizeSession && state.session) {
      await translationPromise;
      stopQuestionSpeech();
      await localizeSessionQuestions(nextLanguage);
      renderQuestion();
    }
  } catch(err) {
    state.data = previousData;
    state.avatarDraft = previousAvatarDraft;
    state.settingsAvatarDraft = previousSettingsAvatarDraft;
...
function speechLangFor(language) {
  return SPEECH_LANGS[language] || `${HTML_LANGS[language] || "en"}-IN`;
}
...
function loadSpeechVoices() {
  ...
  if (!speechVoicesBound && typeof synth.addEventListener === "function") {
...
function pickSpeechVoice(lang) {
  ...
  const languageVoice = voices.find(voice => normalizeSpeechLang(voice.lang) === target) ||
    voices.find(voice => normalizeSpeechLang(voice.lang).startsWith(`${base}-`)) ||
    voices.find(voice => normalizeSpeechLang(voice.lang).split("-")[0] === base);
  if (languageVoice || base !== "en") return languageVoice || null;
  return voices.find(voice => normalizeSpeechLang(voice.lang) === "en-in") ||
    voices.find(voice => normalizeSpeechLang(voice.lang).startsWith("en-")) ||
    null;
}

function questionSpeechKey(question) {
  return `${state.session?.id || "quiz"}:${state.quizIndex}:${question?.id || originalQuestionIndex(question)}`;
}

function updateQuestionSpeechButton() {
  const button = document.querySelector("#readQuestion");
  if (!button) return;
  const active = !!state.speakingQuestionKey && button.dataset.speechKey === state.speakingQuestionKey;
  const label = t(active ? "stopReadingQuestion" : "readQuestion");
...
function stopQuestionSpeech() {
  ...
  state.speakingQuestionKey = null;
  updateQuestionSpeechButton();
}

async function localizedQuestionPrompt(question, language) {
  // Prefer the localized `question.prompt` when present (set by localizeSessionQuestions),
  // otherwise fall back to the original `basePrompt`.
  const source = question?.prompt || question?.basePrompt || "";
  language = language || "English";
  if (!source || language === "English") return source;
  let translated = window.I18N.contentText(language, source);
  if (translated !== source) return translated;
  await translateBatch([source], language);
  translated = window.I18N.contentText(language, source);
  return translated !== source ? translated : source;
}

async function speakQuestionPrompt(question) {
  const synth = speechSynth();
  ...
  const key = questionSpeechKey(question);
  if (state.speakingQuestionKey === key) {
    stopQuestionSpeech();
    return;
  }

  stopQuestionSpeech();
  state.speakingQuestionKey = key;
  updateQuestionSpeechButton();

  const language = state.data?.profile?.preferredLanguage || currentLanguage();
  const text = await localizedQuestionPrompt(question, language);
  if (state.speakingQuestionKey !== key) return;
  if (!text) {
    state.speakingQuestionKey = null;
    updateQuestionSpeechButton();
    return;
  ...
  const lang = speechLangFor(language);
  utterance.lang = lang;
...
async function doStartQuiz() {
  ...
  await localizeSessionQuestions(currentLanguage());
  renderQuestion();
}

async function localizeSessionQuestions(language) {
  if (!state.session?.questions?.length) return;
  language = language || "English";

  const sourceTexts = [...new Set(state.session.questions.flatMap(question => {
    const options = Array.isArray(question.baseOptions) ? question.baseOptions : question.options || [];
    return [
      question.basePrompt || question.prompt,
      ...options,
      question.baseTopic || question.topic,
      question.baseExplanation || question.explanation
    ].filter(Boolean);
  }))];
  try { console.debug && console.debug(`[localizeSessionQuestions] language=${language} sourceTexts=${sourceTexts.length}`); } catch(e){}

  if (language !== "English") await translateBatch(sourceTexts, language);

  state.session.questions = state.session.questions.map(question => {
    const basePrompt = question.basePrompt || question.prompt;
    const baseOptions = Array.isArray(question.baseOptions) ? question.baseOptions : question.options || [];
    const baseTopic = question.baseTopic || question.topic;
    const baseExplanation = question.baseExplanation || question.explanation;
    return {
      ...question,
      basePrompt,
      baseOptions,
      baseTopic,
      baseExplanation,
      prompt: language === "English" ? basePrompt : window.I18N.contentText(language, basePrompt),
      options: language === "English" ? baseOptions : baseOptions.map(option => window.I18N.contentText(language, option)),
      topic: language === "English" ? baseTopic : window.I18N.contentText(language, baseTopic),
      explanation: language === "English" ? baseExplanation : window.I18N.contentText(language, baseExplanation)
    };
  });
  try { console.debug && console.debug(`[localizeSessionQuestions] localized ${state.session.questions.length} questions for ${language}`); } catch(e){}
}
...
function renderQuestion() {
  ...
  // Prefer already-localized fields (set by localizeSessionQuestions);
  // fall back to base values if localized ones are not present.
  const prompt = question.prompt || question.basePrompt;
  const topic = question.topic || question.baseTopic;
  const options = Array.isArray(question.options) ? question.options : (Array.isArray(question.baseOptions) ? question.baseOptions : []);
  const translatedPrompt = language === "English" ? prompt : window.I18N.contentText(language, prompt);
  const translatedTopic = language === "English" ? topic : window.I18N.contentText(language, topic);
  const translatedOptions = language === "English"
    ? options
    : options.map(option => window.I18N.contentText(language, option));

  // Queue the actual texts that will be shown for translation if needed.
  queueTranslation([prompt, topic, ...options]);

  updateActivityHead({ eyebrow: translatedTopic, title: t("decisionCheck") });
  renderLevelTabs(state.selected.lessonId);

  const LABELS = ["A", "B", "C", "D"];
  const speechKey = questionSpeechKey(question);
  const speechLabel = t(state.speakingQuestionKey === speechKey ? "stopReadingQuestion" : "readQuestion");
  ...
  els.activityBody.innerHTML = `
    <div class="quiz-card mcq-immersive">
      ...
      <div class="mcq-question-wrap">
        ...
        <button class="mcq-speak-button ${state.speakingQuestionKey === speechKey ? "speaking" : ""}" id="readQuestion" type="button" data-speech-key="${attr(speechKey)}" aria-label="${attr(speechLabel)}" title="${attr(speechLabel)}">
        ...
        <div class="question-prompt mcq-prompt">${translatedPrompt}</div>
      </div>
  ...
  document.querySelector("#readQuestion")?.addEventListener("click", () => {
...
async function init() {
  ...
  setData(data);
  // Warm up translations (static + curriculum + question bank) for preferred language
  try {
    await prepareLanguage(state.data.profile.preferredLanguage, { full: true });
  } catch (e) { /* ignore */ }
  renderEnergy();
}
